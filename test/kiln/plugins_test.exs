defmodule Kiln.PluginsTest do
  ...
  describe "mix kiln.plugins.doctor" do
    ...
    # The contract check used to be duplicated: the inline copy in
    # field_type_problems/1 had no rescue for a non-atom field_types() entry,
    # so it crashed the entire doctor run with an unrelated FunctionClauseError
    # instead of reporting a clean plugin problem.
    test "reports a plugin problem instead of crashing on a malformed field_types entry" do
      defmodule MalformedFieldTypesPlugin do
        use Kiln.Plugin
        def field_types, do: ["not_a_module"]
      end

      Application.put_env(:kiln_cms, :plugins, [MalformedFieldTypesPlugin])
