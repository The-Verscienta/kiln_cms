defmodule KilnCMSWeb.ExperimentsDeliveryTest do
  ...
  defp experimented_page(ctx, patch \\ %{"fields" => %{"title" => @variant_headline}}) do
    ...
    {experiment, control, treatment} = ExperimentFixtures.running!(page, "page", patch)
    ExperimentFixtures.pin!(treatment, [control], ctx.org_id)
  ...
  describe "the headless surface" do
    ...
    # Headless keeps `public` caching because the key belongs to the caller —
    # so the cache can key on it, which is what `Vary` says.
    test "advertises the variant and varies on the key", ctx do
      %{page: page} = experimented_page(ctx)

      conn = get(ctx.conn, "/api/content/page/#{page.slug}?variant_key=visitor-1")

      assert ["X-Kiln-Variant-Key"] = get_resp_header(conn, "vary")
    ...
    test "the ETag carries the variant, so a 304 cannot cross arms", ctx do
      %{page: page, control: control, treatment: treatment} = experimented_page(ctx)

      treatment_etag =
        ctx.conn
        |> get("/api/content/page/#{page.slug}?variant_key=v1")
        |> get_resp_header("etag")

      ExperimentFixtures.pin!(control, [treatment], ctx.org_id)
      ...
      assert treatment_etag != control_etag
    end
  ...
  describe "invariant 1: a variant is never fired" do
    test "the stored artifact stays canonical after a variant is served", ctx do
      %{page: page} = experimented_page(ctx)
      ...
      {:ok, artifact} =
        KilnCMS.Firing.Engine.read(ctx.org_id, :page, page.id, :json)
  ...
  describe "invariant 2: a variant never writes the document" do
    test "the record, its blocks and its version chain are untouched", ctx do
      %{page: page} = experimented_page(ctx)
