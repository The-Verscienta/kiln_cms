...
**Since that snapshot:** the Phase 3 content editor shipped (TipTap + drag-and-drop blocks + live preview — `KilnCMSWeb.ContentEditorLive` plus its `content_editor/` components), as did AshAdmin actor wiring (`KilnCMSWeb.AshAdmin.ActorPlug`), image variants and the S3 adapter. DaisyUI is removed; only its class *naming* survives, in a self-owned kit ([`docs/design-language.md`](docs/design-language.md)). Repo lives at a **spaceless path** (`~/Github/kiln_cms`): `picosat_elixir`'s Makefile cannot build under a path with a space, such as iCloud Drive's (see [`CONTRIBUTING.md`](CONTRIBUTING.md)).
...
> **Checklist reconciled against main, 2026-08-28.** Caution on issue history: PR #106 ("complete remaining roadmap issues") merged **prematurely with only its first commit** (#46, notification prefs) — the other ~19 issue commits never reached main, though several were independently rebuilt later (plugins #262/#263, static export, a11y audits, localization #272). Issues whose scope was still genuinely missing were **reopened**: #42, #45, #48, #51, #53, #56, #57, #59, #60, #62, #65. **Every one of those has since been rebuilt on main and closed again except #59** — the beta rounds, which need people rather than code. #62 is closed by both the design doc it asked for (`docs/advanced-analytics-plan.md`) and five of its six implementation phases (#618–#622; #623 was closed as a deliberate non-build, see Stretch below), and #65 by the spike in `docs/mobile-admin-spike.md` plus the PWA it recommends.
...
- [x] Configure native `Phoenix.PubSub` (PG2, Phoenix default). Dragonfly kept as an *optional* Compose profile only (D1/D2). In-BEAM caching followed as Cachex (`KilnCMS.Cache`, the delivery hot path)
...
- [x] Define core Ash Resources: `MediaItem`, `Page`, `Post`, with **embedded `Block` resources** (D3) + version resources (via PaperTrail). `User` (with auth) followed in `KilnCMS.Accounts`, and the D6 tenant boundary is `Organization` with Ash's `:attribute` strategy (#336, strict since #419)
...
- [x] Ash Policies for RBAC — `role` attribute (`:admin`/`:editor`/`:viewer`, defaults `:viewer`) enforced by `Ash.Policy.Authorizer` on Page/Post/MediaItem: published content world-readable, unpublished editors-only, authoring/workflow editor+admin, hard-delete admin-only; User policies let admins manage users and users self-read/change-password. PaperTrail `Version` resources are editor/admin-only via the shared `KilnCMS.CMS.VersionPolicies` mixin. Covered by `policies_test.exs`, `version_policies_test.exs`, and field-policy/auth coverage in `user_auth_test.exs`. AshAdmin actor wiring followed (#24): `KilnCMSWeb.AshAdmin.ActorPlug` defaults the dev-only AshAdmin actor to the signed-in user
...
- [x] Integrate **AshPaperTrail** on Page/Post for full history (embedded Blocks are versioned with the parent — D3). Restore followed as the `restore_version` action (below)
...
- [x] **Example frontend consumers (#36)** — [`examples/astro-blog/`](https://github.com/The-Verscienta/kiln_cms/tree/main/examples/astro-blog): a runnable Astro static site that builds a small blog **entirely from the headless API** (no DB/shared code). Discovers content via `GET /sitemap.xml`, fetches each document's structured artifact via `GET /api/content/:type/:slug?surface=json` (the v2 delivery API, D9), renders the typed blocks + Portable Text to HTML client-side (a faithful port of the server renderers), and demonstrates GraphQL search via `POST /gql` (`searchPosts`). Includes a full headless setup walkthrough; [`examples/README.md`](https://github.com/The-Verscienta/kiln_cms/blob/main/examples/README.md) indexes the public delivery surfaces. Build verified against the seeded `welcome` page + `hello-world` post.
- [x] **API documentation (OpenAPI/Swagger via ash_json_api) (#37)** — the AshJsonApi-generated **OpenAPI 3 spec** (`/api/json/open_api`) and its interactive **Swagger UI** (`/api/json/swaggerui`) are now published in **all environments** (dev + prod), not dev-only. `KilnCMSWeb.OpenApi.modify/3` (wired as `:modify_open_api`) enriches the generated spec with a titled `info` block + auth/usage description (covering bearer auth, pagination/filtering, GraphQL, webhooks, preview), concrete `servers`, and an **optional** bearer requirement (`security: [%{}, %{"bearerAuth" => []}]` — published content is anonymous, a token only widens access to drafts). Swagger UI runs under a dedicated `:swagger_ui` pipeline + CSP (cdnjs bundle allowed, inline boot script nonced). **Headless sign-in:** a new `KilnCMSWeb.ApiAuthController` (`POST /api/auth/sign_in`, `:auth` rate-limit bucket) exchanges email+password for the AshAuthentication user **JWT** as JSON (`{token, user}`) — the server-to-server way to get a bearer token (the browser auth flow is session-based). Invalid creds return a generic 401 (no user enumeration); the endpoint is documented in the OpenAPI spec via `auth_paths/0`. **Two-factor accounts finish at a second step (#726):** the password alone answers `200 {two_factor_required, pending_token, expires_in}` rather than `201` + JWT, and `POST /api/auth/sign_in/verify` redeems that pending blob (encrypted, single-use, `KilnCMS.Accounts.PendingSignIn`) plus a TOTP or recovery code — sharing the browser prompt's per-account second-factor budget. A top-level [`docs/api.md`](docs/api.md) indexes every headless surface (auth/sign-in, JSON:API, GraphQL, fired artifacts, webhooks, preview tokens, rate limits). Covered by `api_explorer_routes_test.exs` (spec + Swagger UI reachable with `dev_routes` false; spec carries the bearer scheme + both sign-in paths + core content paths) and `api_auth_controller_test.exs` (token issuance/usage, the two-factor exchange, replay refusal, generic 401, 422).
...
- [x] CDN integration strategy for media (#42) — media is served from object storage with a CDN in front, so image bytes never touch the app. Setting `S3_BUCKET` switches adapters and makes `S3_PUBLIC_BASE_URL` required: that hostname **is** the CDN origin, and every URL the CMS emits — original and variant alike — is that base plus the storage key. `KilnCMS.Storage.S3` writes `Cache-Control: public, max-age=31536000, immutable` as object metadata at `PUT` time. Caching that hard is safe because storage keys are **write-once UUIDs**: an in-admin edit writes a new key and repoints the item rather than overwriting in place, so a URL always denotes the same bytes and there is no CDN purge step to wire up. Per-provider recipes (R2, CloudFront, B2/Wasabi, MinIO) in `docs/media-pipeline.md`. API responses got their own `Cache-Control` earlier via #188.
...
- [x] Basic analytics (page view tracking + simple dashboard) — **privacy-first**: a separate `KilnCMS.Analytics` domain with a `ContentView` upsert counter (one row per content item, atomic `views + 1` increment, `last_viewed_at`) — **no IPs/user-agents/cookies/PII**. Public delivery records a view best-effort (`ContentController`, failures never break delivery). Dashboard at `/editor/analytics` (`KilnCMSWeb.AnalyticsLive`, editor/admin only) shows total views + most-viewed content with titles. Covered by `analytics/content_view_test.exs` + `analytics_live_test.exs`. **Extended (#45):** a `ContentViewDay` per-day bucket (400-day retention, nightly AshOban purge) gives the dashboard a URL-held 7d/30d trend with an inline-SVG `bar_chart/1`, and every delivered view emits `[:kiln_cms, :analytics, :view]` for external sinks — emitted before the DB write is dispatched, so it tracks traffic even when the write is shed under load (`docs/observability.md`).
- [x] Email notifications (Swoosh + Oban) for workflow events — `KilnCMS.Notifications` mirrors the webhook pipeline: a `Changes.NotifyWorkflowEmail` change (on `submit_for_review`/`publish`/`publish_scheduled` for Page+Post) resolves recipients as a system read and enqueues `Notifications.WorkflowMailWorker` Oban jobs. **Submit-for-review** emails every admin (minus the submitter); **publish** emails the author (covers scheduled, actor-less publishing). Swoosh via the existing `KilnCMS.Mailer`. Covered by `notifications_test.exs`. **Both TODOs are now done:** changes-requested ships as the `returned_to_draft` event (the worker also carries `comment_added`/`comment_resolved`/`comment_mention` from the editorial-comments arc, #801/#802), and per-user notification preferences are self-service via `:update_notification_prefs` on `KilnCMS.Accounts.User` (`/editor/settings`).
...
- [x] Theming / white-label potential (#48) — `KilnCMS.Branding` resolves white-label tokens **per-org**, which is exactly the shape strict multi-tenancy made natural. Three layers, most specific first: the site's editor-managed `KilnCMS.CMS.SiteBranding` row, the instance-wide operator default (`SITE_NAME` / `BRAND_LOGO_URL` / `BRAND_PRIMARY_COLOR`), then the stock KilnCMS defaults — a nil or blank value at any layer falls through, so clearing a field in the editor restores the operator default rather than blanking the header. `for_org/1` sits on the public delivery hot path and caches the *resolved struct* rather than the row (the cache never stores a nil, and most sites have no branding row), and never writes. **Scope note:** this is console/chrome branding. A theme system for the *public* site is a separate and still-open question — #1318.
...
- [x] Security: Sobelow (**done** — wired into precommit/CI), **nonce-based CSP** on the browser pipeline (**done** — browser-verified), policy coverage (**done** — `docs/policy-matrix.md` + policy tests; three full security audits since June 2026), rate limiting tests (**done**), and the automated **dependency-audit gate** (**done** — `{:mix_audit, "~> 2.1"}` runs as `mix deps.audit` in `precommit` and in its own `deps_audit` CI job; #51 closed). **Tracked separately now:** #1309 audits the existing `authorize?: false` sites — the `mix kiln.authz.check` gate that stops *new* ones landed, the sweep of the existing ones has not.
...
- [x] Full ExDoc documentation + guides — rebuilt after #106 and shipped (#53): `docs/0` in `mix.exs` wires every `docs/` guide plus the root markdown into a grouped sidebar (`extras`/`groups_for_extras`) alongside the module reference, and a dedicated CI job runs `mix docs --warnings-as-errors`.
- [x] This project plan kept up-to-date as living doc — reconciled 2026-08-28 against the tracker (every checkbox re-checked against issue state and against the code, not against the previous reconciliation); see also the realized-history note in `kiln-cms-plan-v2.md`.
...
- [x] Monitoring setup (#56) — Sentry (errors) + OpenTelemetry (tracing) wired, env-gated no-ops by default (`docs/observability.md`); LiveDashboard available. **Three** probes now back the healthcheck and external monitoring (`KilnCMSWeb.HealthController`): `GET /live` (liveness, no DB check — the *only* one that should ever trigger a restart, #816), `GET /up` (readiness, `SELECT 1`), and `GET /ready`, which returns a JSON snapshot carrying `db` plus `oban.available`/`retryable`/`backlog` counted straight from `oban_jobs`, so it needs no Oban Pro/Met dependency. Alert rules — DB connectivity, Oban queue depth, readiness flapping — are documented in `docs/observability.md` with a Prometheus rule sketch.
- [x] Backup strategy (Postgres + media) (#57) — `scripts/backup.sh` and `scripts/restore.sh`, plain shell over `pg_dump`/`tar` so they behave identically on a laptop and on the VPS, with `KilnCMS.Backups` carrying the manifest and the scheduled worker. `docs/backups.md` is the runbook: what is covered, on what schedule, where copies live, how to restore, and the quarterly restore drill.
...
### Stretch / Post-v1.0
- [x] AI content generation assistant (block-level prompts via LLM) (#60) — `KilnCMS.Assist`: the author picks one block and one of six actions (Draft, Continue, Summarize, Improve, Shorten, Expand) and gets prose back to accept or throw away. **Off by default** (`generator: nil`) — a default install renders no assist control and nothing leaves the deployment. The shipped adapter is provider-agnostic via `req_llm`, and every provider's `base_url` is overridable, so an on-prem endpoint needs no Kiln code; API keys are resolved by `req_llm` from its own environment and never enter Kiln's config, database or release env. `egress?/0` reports which choice the operator actually made — a third-party provider logs a warning at boot and shows a standing notice next to the control. Guide: `docs/ai-assist.md`. The metadata half of the same surface is `KilnCMS.Seo` (`docs/seo.md`), with a separate switch. Adjacent AI surfaces: MCP server (`docs/mcp.md`), RAG (`docs/rag.md`), llms.txt/GEO delivery.
- [~] Real-time collaborative editing — built, but **not on in production**: Yjs CRDT with server-side checkpoint materialization (#258/#261), the BEAM rendering Yjs→HTML with no JS step. It sits behind the `:collab_prototype` flag — on in dev and test, **off in prod** — and the flag is VM-global rather than per-document, so it is a prototype rather than a shipped feature. #1324 tracks the decision to either finish it or label it plainly; until then, do not cite live co-editing as shipped.
- [x] Advanced reporting / content analytics — governance dashboard shipped (`docs/governance-dashboard.md`); time-series trends + view telemetry shipped (#45). The plan in `docs/advanced-analytics-plan.md` (#62) is **built**: phases 1–5 shipped as #618–#622 — a shared CSV writer plus streamed CSV/JSON download (`KilnCMS.Analytics.Export`), privacy-respecting referrer attribution (`Analytics.ReferrerDay`) surfaced on `AnalyticsLive` with low-count suppression, and funnel definitions (`Analytics.Funnel`/`FunnelStep`) with admin CRUD and a report derived from `content_view_days`. Phase 6 (#623, month roll-up for buckets past the 400-day window) was **closed as a deliberate non-build**: it is the only phase that *loses* data, the 400-day window already answers every year-over-year question asked so far, and the issue's own guidance was not to build it speculatively. Re-open it if a deployment actually needs multi-year trend history.
- [x] Robust **Plugin / Module System** — `Kiln.Plugin` (D18) complete incl. the custom field-type registry (`Kiln.FieldType`, #266). Catalog metadata, `Kiln.Plugins.manifests/0` and `mix kiln.plugins.list` shipped the discovery half; installing code into a *running* instance was answered **no, by design** (`docs/plugin-extensibility.md` §1). #333 now tracks only the separate, parked question of sandboxed runtime-code extension points (WASM / out-of-process).
