...
- [x] Performance profiling (LiveDashboard, Telemetry metrics for editor actions) — **#41 done.** `KilnCMSWeb.EditorTelemetry.span/3` times each editor action and emits `[:kiln_cms, :editor, …]` events — `:save` (explicit Save), `:autosave` (debounced draft), `:publish` (publish transition), and `:workflow` (submit/return/unpublish/archive) — with a `:duration` measurement (native units) and `kind`/`action`/`result` metadata (result derived from the Ash return tuple, so success/error latency splits cleanly). Matching `Telemetry.Metrics` (a `summary` + `counter` per event, tagged) added to `KilnCMSWeb.Telemetry.metrics/0`, so they surface on the LiveDashboard **Metrics** page under `kiln_cms.editor.*` with no extra wiring; a documented Prometheus/Grafana export path is in `docs/observability.md`. Covered by `editor_telemetry_test.exs` (attaches a handler, asserts save/autosave/publish fire).
