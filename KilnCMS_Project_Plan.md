...
- [x] Image processing pipeline (`Image`/libvips + **Oban job for variants**) — `KilnCMS.ImageProcessor` (libvips) reads dimensions + builds responsive variants (thumb 400 / medium 1024, never upscales). Generation runs **off the upload request** in `KilnCMS.Media.VariantWorker`: `MediaLive` stores the original, creates the item, copies the upload to a temp file and enqueues the worker, which processes, persists variant blobs via `KilnCMS.Storage`, and writes `width`/`height`/`variants` back. Non-raster uploads / deleted items degrade gracefully; the original is always served. Covered by `media/variant_worker_test.exs` + the `media_live_test` upload test (drains Oban). **Follow-up:** multi-node variant generation should fetch the original from `Storage` (a `Storage.fetch/1`) instead of a local temp hand-off; live-refresh the library when processing completes (PubSub).
...
- [~] **Real-time Visual Preview** — `PageEditorLive` has a **side-by-side live preview pane** rendered from the form state via the shared `KilnCMSWeb.BlockComponents` (textarea + TipTap edits update it live, debounced). Covered by `editor_live_test.exs` + browser-verified. **Still TODO (full D1 architecture):** PubSub-decoupled preview (pop-out window / signed iframe for public-site fidelity) + Presence for collaboration
...
- [x] Content listing page — `EditorLive` at `/editor`: lists **pages and posts** with type label + status badges, create-new (page/post), inline publish/unpublish, edit links, status filter + title search. **TODO:** bulk actions
...
- [x] PostgreSQL full-text search on content — a denormalized `search_text` (title + SEO + extracted block text, maintained by `Changes.SetSearchText`) + a `search` read action using `to_tsvector`/`plainto_tsquery`, exposed as `CMS.search_pages`/`search_posts`. Visibility-aware (goes through the read policy). Shared `CMS.BlockText` helper (also powers `word_count`). Covered by `content_search_test.exs`. Backed by a **GIN functional index** (`{pages,posts}_search_text_gin_index`) whose `to_tsvector('english', coalesce(search_text, ''))` expression matches the `search` filter exactly, so the planner uses it instead of scanning. **Relevance-ranked** — results sort by `ts_rank` desc (newest as tiebreak) via an internal `search_rank` calculation threaded the query term. **Follow-up:** optional highlighting; a materialized/generated `tsvector` column if rank computation ever shows up in profiles.
- [ ] Optional: Meilisearch integration + indexing jobs (Oban)
- [~] Caching strategy — **Cachex (in-BEAM)** caches published content on the delivery hot path: `KilnCMS.Cache.fetch_published/3` caches the record per `{type, slug}` (60-min safety-net TTL; nils never cached so new content shows immediately), used by `ContentController`. Precise invalidation via a global `Changes.BustContentCache` on content create/update/destroy that busts only when published content is involved (publish/edit-published/unpublish/archive/delete), skipping draft-only writes (e.g. autosave). Toggle with `config :kiln_cms, KilnCMS.Cache, enabled: false`. Covered by `cache_test.exs` + `content_cache_test.exs` (serves from cache, busts on write). **Rate limiting on APIs (Hammer/ETS)** already wired (`KilnCMSWeb.RateLimit`). Dragonfly stays deferred (D2). **Still TODO:** cache the enriched block media too; per-key (not full-clear) invalidation.
