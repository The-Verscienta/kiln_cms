...
- [x] Enable **AshJsonApi** (router at `/api/json` + OpenAPI/Swagger UI) — exposed per resource (D7). **Filtering/sorting/pagination tuned (#33):** Page/Post/MediaItem now expose `index :read` + `get :read` routes (Post also `/posts/published`); the primary `:read` actions are paginated (offset + keyset, `default_limit: 25`, `max_page_size: 100`, `countable`, `required?: false` so internal `CMS.list_*` callers still get plain lists). Filtering (`filter[...]`), sorting (`sort=`) and pagination (`page[...]`) derive from the public fields. Query params documented in `docs/json-api.md`; covered by `json_api_test.exs` (filter combinations, sort asc/desc, offset paging + count/next-link, get-by-id, draft visibility via bearer, media filtering). **OpenAPI docs published (#37):** the machine-readable OpenAPI 3 spec at `/api/json/open_api` is now served in **all** environments (previously dev-only), enriched by `KilnCMSWeb.OpenApi` with the title, app version and a description covering authentication (the auto-generated `bearerAuth` JWT scheme), pagination and outbound webhooks. The interactive Swagger UI explorer stays dev-only (inline scripts/CDN assets vs. the strict prod CSP) — prod consumers render the published spec with any external viewer (Redoc/Swagger UI/Postman) or generate clients from it. Covered by `api_explorer_routes_test.exs` (spec published without `dev_routes`; asserts title/version/description, bearer scheme + global security, and the Page/Post/MediaItem paths).
