...
the standing guide; the per-release checklists that used to be the only deploy
material (`deploy-p2.md`, `deploy-p3.md`, `deploy-staging.md`,
`deploy-write-visual-editing.md`) are rehearsal notes for specific feature
batches and are kept under *Audits & release checklists* — read them for that
release's gotchas, not for how to deploy at all.
...
1. **`bin/migrate`** runs `KilnCMS.Release.migrate/0` — every pending Ecto
   ...
   `Ecto.Migrator` takes a database advisory lock, so this stays safe when
   several replicas start at once: one migrates, the others wait, then all
   serve.
...
Rolling back a migration is a manual step and never automatic:

```bash
docker exec -it <container> /app/bin/kiln_cms eval 'KilnCMS.Release.rollback(KilnCMS.Repo, <version>)'
```
...
|----------|---------|----------------------|---------------|
| `GET /live` | `200 OK` iff the endpoint is serving HTTP; connection refused otherwise | **No** | Anything that **restarts** the container on failure: the Docker `HEALTHCHECK` (already wired), Kubernetes liveness, Coolify's health check, systemd watchdogs. |
...
Platforms with their own health-check UI (Coolify, Kubernetes) ignore it; set
the same path and a start period of at least a minute there.
...
1. Register your own account at `https://<PHX_HOST>/register` (or set
   `config :kiln_cms, :registration_enabled, false` *after* this step if you
   want invite-only sign-up).
...
- forward `X-Forwarded-For` (and `X-Forwarded-Proto`), and you must name it
  in `TRUSTED_PROXIES` (above), or rate limiting keys on the proxy's address;
...
If a browser can load the editor but every LiveView shows "disconnected" or
the page reloads in a loop, it is almost always `PHX_HOST` (wrong host, or a
scheme baked in) or the proxy not upgrading WebSockets.
...
| **Meilisearch** (typo-tolerant, faceted search) | `MEILI_URL` (+ `MEILI_MASTER_KEY`, `MEILI_INDEX`) — then `mix kiln.meili.reindex` once | `search` | Postgres full-text search is the default and stays available. Run Meilisearch with `MEILI_ENV=production` and a real master key (16+ bytes) — it refuses to start otherwise. [`meilisearch.md`](meilisearch.md). |
| **Dragonfly / Redis** (shared cache) | Nothing yet — see note | `cache` | Kiln's caches are in-process by decision (D2: minimal ops); the multi-node cache tier is a config seam that **no shipped adapter uses**. Cross-node cache busts already travel over native PubSub. The profile is provided so a cluster that adds a tier-2 adapter has somewhere to point it; on a single node it does nothing for you. Don't run it expecting a speedup. |

Multi-node itself needs no broker: Phoenix PubSub is native, Oban is
Postgres-backed, and node discovery is `DNS_CLUSTER_QUERY`. What a cluster
*does* need is a shared media backend (object storage) — Local uploads on one
node are invisible to the others.
...
does not run backups, and keeps Postgres on the compose network with TLS off
because the traffic never leaves the host.

```bash
# .env.prod: SECRET_KEY_BASE, TOKEN_SIGNING_SECRET, PHX_HOST, POSTGRES_PASSWORD,
# TRUSTED_PROXIES, plus any optional variable from environment-variables.md
...
you add there reaches the container verbatim. That pass-through is
deliberate: several variables are **presence-checked** and treat an empty
string as "set" (`S3_BUCKET=` would switch the storage adapter to S3 with no
bucket; `MEILI_URL=` would enable a backend with no server), so the compose
file lists no optional variable with an empty default — absent stays absent.
The file's header enumerates what it reads; every name is a row in
[`environment-variables.md`](environment-variables.md).

Put a proxy in front (Caddy is the least configuration:
`reverse_proxy app:4000` behind an automatic-TLS site block) and set
`TRUSTED_PROXIES` to the compose network's range.
...
`/up`, `initialDelaySeconds` of at least 60 on liveness (cold boot plus
migrations). Several replicas may start together; the migration lock handles
it. Use object storage for media.
...
- Registration closed (`config :kiln_cms, :registration_enabled, false`) once
  the first admin exists, unless the site is meant to be open.
