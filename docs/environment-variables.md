...
> `KILN_LINK_CHECK_USER_AGENT`, `KILN_TASK_DIGEST_CRON`, `PRESENTATION_PREVIEW_URL`, the `KILN_UPDATE_*`
...
| Variable | Default | Purpose | Where it's read |
|----------|---------|---------|-----------------|
| `API_DOCS_ENABLED` | on outside prod, **off in prod** | Serve `GET /api/json/open_api` and `GET /api/json/swaggerui`. Turn it on for a deployment that publishes a public API. | [`config/runtime.exs:737`](../config/runtime.exs#L737) |
...
| Variable | Default | Purpose | Where it's read |
|----------|---------|---------|-----------------|
| `SITE_NAME` | `KilnCMS` | Instance name in the admin chrome, page titles and outbound email. Also the default provenance `signer` identity when `KilnCMS.Provenance`'s `:signer` is unset. | [`config/runtime.exs:748`](../config/runtime.exs#L748) |
| `BRAND_LOGO_URL` | unset | Logo shown in the admin chrome and on branded error pages. If the host differs from the site's origin it must also be in `CSP_IMG_SRC`, or the browser blocks the image. | [`config/runtime.exs:749`](../config/runtime.exs#L749) |
| `BRAND_FAVICON_URL` | unset | Favicon URL. Same `CSP_IMG_SRC` caveat as the logo. | [`config/runtime.exs:750`](../config/runtime.exs#L750) |
| `BRAND_PRIMARY_COLOR` | unset | Hex colour (`#1d4ed8`) driving the emitted OKLCH theme tokens. Anything that isn't a hex colour is **ignored with a warning** rather than interpreted, since the value feeds contrast computation. | [`config/runtime.exs:751`](../config/runtime.exs#L751) |

## Optional — Unsplash (media library)

| Variable | Default | Purpose | Where it's read |
|----------|---------|---------|-----------------|
| `UNSPLASH_ACCESS_KEY` | unset | The Unsplash API **Access Key** (not the secret key, which this never needs). Setting it adds the Unsplash tab to the media library, where an editor can search and import stock photos; unset, the tab never renders and no request reaches Unsplash. Read in every environment. **Where to get it:** register an application at [unsplash.com/oauth/applications](https://unsplash.com/oauth/applications). A new app is in Demo mode (50 requests/hour) — enough to evaluate the tab, but production traffic needs Unsplash to approve the application. | [`config/runtime.exs:64`](../config/runtime.exs#L64) |

## Optional — database TLS

| Variable | Default | Purpose | Where it's read |
|----------|---------|---------|-----------------|
| `DATABASE_SSL` | `true` | Encrypt the Postgres connection. Set to an off-spelling only for a provider that genuinely cannot offer TLS — an unrecognized value keeps TLS on rather than silently downgrading to plaintext (#606). | [`config/runtime.exs:605`](../config/runtime.exs#L605) |
| `DATABASE_SSL_CACERTFILE` | unset | Path to the provider's CA bundle. When set, the server cert is verified (`verify_peer`); unset — or blank, like every variable above — leaves the connection encrypted but `verify_none`. | [`config/runtime.exs:613`](../config/runtime.exs#L613) |
...
| Variable | Default | Purpose | Where it's read |
|----------|---------|---------|-----------------|
| `S3_BUCKET` | unset | Enables the S3 adapter. Leave unset to use local storage. | [`config/runtime.exs:868`](../config/runtime.exs#L868) |
| `S3_PUBLIC_BASE_URL` | — | Public base URL objects are served from — the CDN hostname, including the bucket path if the provider's URLs carry one. **Required when `S3_BUCKET` is set** (raises otherwise). | [`config/runtime.exs:878`](../config/runtime.exs#L878) |
| `AWS_ACCESS_KEY_ID` | — | S3 access key. **Required when `S3_BUCKET` is set** (`fetch_env!`). | [`config/runtime.exs:904`](../config/runtime.exs#L904) |
| `AWS_SECRET_ACCESS_KEY` | — | S3 secret key. **Required when `S3_BUCKET` is set** (`fetch_env!`). | [`config/runtime.exs:905`](../config/runtime.exs#L905) |
| `AWS_REGION` | `us-east-1` | Region. Use `auto` for Cloudflare R2; a real region for B2/Wasabi/AWS. | [`config/runtime.exs:907`](../config/runtime.exs#L907) |
| `S3_ACL` | unset | Per-object canned ACL (e.g. `public_read`). Only needed if the bucket isn't public at the bucket level. | [`config/runtime.exs:885`](../config/runtime.exs#L885) |
| `S3_PRIVATE_BUCKET` | unset | A separate bucket for gated documents (#481) — this app's own AWS credentials read it directly, so it needs no public-read config, CDN, or public-base-URL equivalent. Without it, gating a document is refused rather than silently falling back to the public bucket. | [`config/runtime.exs:896`](../config/runtime.exs#L896) |
| `S3_ENDPOINT_HOST` | unset | Custom endpoint host for non-AWS stores (R2/B2/Wasabi/MinIO). Leave unset for AWS S3. | [`config/runtime.exs:870`](../config/runtime.exs#L870) |
| `S3_ENDPOINT_SCHEME` | `https://` | Scheme for the custom endpoint. | [`config/runtime.exs:913`](../config/runtime.exs#L913) |
| `S3_ENDPOINT_PORT` | `443` | Port for the custom endpoint. | [`config/runtime.exs:915`](../config/runtime.exs#L915) |
...
| Variable | Default | Purpose | Where it's read |
|----------|---------|---------|-----------------|
| `BACKUP_ENABLED` | `true` | Set to an off-spelling to disable the **in-app** backup path — the console explains why and the button is disabled. Cron backups on the host are unaffected, which is the point: turning off the button is not turning off backups. Parsed by the shared [on/off rules](#onoff-variables). | [`config/runtime.exs:789`](../config/runtime.exs#L789) |
| `BACKUP_DIR` | `/var/backups/kiln` | Where backups land, for both paths. | [`config/runtime.exs:796`](../config/runtime.exs#L796) |
| `BACKUP_KEEP_DAYS` | `14` | Local retention in days, enforced by both paths. A non-positive or unparseable value keeps the default and warns — read literally, `0` would delete the backup it had just taken. | [`config/runtime.exs:809`](../config/runtime.exs#L809) |
| `BACKUP_STALE_AFTER_HOURS` | `36` | How old the newest backup may be before the console warns and the overview shows a red strip. Deliberately longer than a daily cadence: a warning that fires because a nightly job ran at 03:20 instead of 03:17 is one an admin learns to ignore. | [`config/runtime.exs:833`](../config/runtime.exs#L833) |
| `MEDIA_DIR` | unset | Uploads root to archive — **Local storage adapter only**. Leave unset on S3/R2, where the bucket is backed up provider-side: tarring a directory that doesn't hold the media produces an archive that looks like a media backup and restores nothing. | [`config/runtime.exs:793`](../config/runtime.exs#L793) |
...
| Variable | Default | Purpose | Where it's read |
|----------|---------|---------|-----------------|
| `MAIL_MODE` | unset | `smtp` = relay through an SMTP server; `direct` = deliver straight to each recipient domain's MX hosts (built-in MTA, no relay). Anything else raises at boot. | [`config/runtime.exs:1078`](../config/runtime.exs#L1078) |
| `MAIL_FROM_EMAIL` | unset | From address for all outbound mail. **Required when `MAIL_MODE=direct`** (raises otherwise) — its domain is the sending/DKIM domain. | [`config/runtime.exs:1084`](../config/runtime.exs#L1084) |
| `MAIL_FROM_NAME` | `KilnCMS` | Display name for the From address. | [`config/runtime.exs:1164`](../config/runtime.exs#L1164) |
| `SMTP_HOST` | unset | Relay host. **Required when `MAIL_MODE=smtp`**; setting it without `MAIL_MODE` also selects smtp mode. | [`config/runtime.exs:1078`](../config/runtime.exs#L1078) |
| `SMTP_PORT` | `587` | Relay port. 587 (STARTTLS) is the right default, and 25 belongs to `MAIL_MODE=direct`. **465 will not work**: that port expects implicit TLS from the first byte, and the adapter is configured for STARTTLS only (`tls:`, never gen_smtp's `ssl:`) with no environment variable to change it — a relay that offers both ports should be pointed at 587. | [`config/runtime.exs:1124`](../config/runtime.exs#L1124) |
| `SMTP_USERNAME` | unset | Relay username (`auth: :always`). **From the relay provider's dashboard** — providers name the pair differently: Postmark issues one Server API Token used as *both* username and password; SES issues dedicated SMTP credentials, which are **not** your AWS access keys; Gmail requires an App Password rather than the account password. | [`config/runtime.exs:1125`](../config/runtime.exs#L1125) |
| `SMTP_PASSWORD` | unset | Relay password; see `SMTP_USERNAME` for where it comes from. | [`config/runtime.exs:1126`](../config/runtime.exs#L1126) |
| `SMTP_TLS` | `true` | STARTTLS to the relay. Set to an off-spelling only for a local dev/test relay. | [`config/runtime.exs:1080`](../config/runtime.exs#L1080) |
| `SMTP_TLS_VERIFY` | `true` | Verify the relay's certificate against [CAStore](https://hex.pm/packages/castore)'s bundle, with SNI. Set to an off-spelling for a relay with a self-signed or mismatched certificate: the connection stays encrypted but the peer is not verified (`verify_none`). | [`config/runtime.exs:1107`](../config/runtime.exs#L1107) |
| `MAIL_HELO_HOST` | `PHX_HOST` | Direct mode only: HELO/EHLO hostname. Deliverability requires the sending IP's PTR record to resolve to this name. | [`config/runtime.exs:1140`](../config/runtime.exs#L1140) |
...
| Variable | Default | Purpose | Where it's read |
|----------|---------|---------|-----------------|
| `MEILI_URL` | unset | Meilisearch server URL. Enables the backend when set. | [`config/runtime.exs:921`](../config/runtime.exs#L921) |
| `MEILI_MASTER_KEY` | unset | Meilisearch API master key. | [`config/runtime.exs:928`](../config/runtime.exs#L928), [`lib/kiln_cms/search/meilisearch.ex:14`](../lib/kiln_cms/search/meilisearch.ex#L14) |
| `MEILI_INDEX` | `kiln_content` | Index name. | [`config/runtime.exs:929`](../config/runtime.exs#L929) |
...
| Variable | Default | Purpose | Where it's read |
|----------|---------|---------|-----------------|
| `SEO_MODEL` | unset | `req_llm` model spec, e.g. `ollama:llama3.1` or `anthropic:claude-sonnet-5`. Enables drafting when set. | [`config/runtime.exs:934`](../config/runtime.exs#L934) |
| `SEO_GENERATOR` | `KilnCMS.Seo.Generator.ReqLLM` | Override the adapter module with your own `KilnCMS.Seo.Generator`. | [`config/runtime.exs:943`](../config/runtime.exs#L943) |
...
| Variable | Default | Purpose | Where it's read |
|----------|---------|---------|-----------------|
| `ASSIST_MODEL` | unset | `req_llm` model spec, e.g. `ollama:llama3.1`. Enables block assist when set. | [`config/runtime.exs:961`](../config/runtime.exs#L961) |
| `ASSIST_GENERATOR` | `KilnCMS.Assist.Generator.ReqLLM` | Override the adapter module with your own `KilnCMS.Assist.Generator`. | [`config/runtime.exs:965`](../config/runtime.exs#L965) |
...
| Variable | Default | Purpose | Where it's read |
|----------|---------|---------|-----------------|
| `ASK_MODEL` | unset | `req_llm` model spec, e.g. `ollama:llama3.1`. Enables generated answers when set. | [`config/runtime.exs:995`](../config/runtime.exs#L995) |
| `ASK_GENERATOR` | `KilnCMS.Ask.Generator.ReqLLM` | Override the adapter module with your own `KilnCMS.Ask.Generator`. | [`config/runtime.exs:997`](../config/runtime.exs#L997) |
...
| Variable | Default | Purpose | Where it's read |
|----------|---------|---------|-----------------|
| `OEMBED_ENABLED` | `false` | Resolve oEmbed metadata so embeds render as cards. | [`config/runtime.exs:1007`](../config/runtime.exs#L1007) |
| `OEMBED_PROVIDERS` | unset (all) | Comma-separated provider names to **narrow** the built-in list. Cannot add one — a new provider is a host this server dials, so it is a code change. | [`config/runtime.exs:1019`](../config/runtime.exs#L1019) |
...
| Variable | Default | Purpose | Where it's read |
|----------|---------|---------|-----------------|
| `SENTRY_DSN` | unset | Sentry DSN (`https://<publickey>@o0.ingest.sentry.io/0`, or your own host when self-hosting). Enables error reporting when set. **Where to get it:** the Sentry project's Settings → Client Keys (DSN). It is not confidential in the strict sense — browser SDKs ship it publicly — but it does authorize writes to the project, so handle it like a credential anyway. | [`config/runtime.exs:101`](../config/runtime.exs#L101) |
| `SENTRY_ENV` | `config_env()` | Environment name tag for Sentry events. | [`config/runtime.exs:104`](../config/runtime.exs#L104) |
| `RELEASE_VSN` | unset | Release version tag (set automatically by the release runtime) to pin regressions to a deploy. | [`config/runtime.exs:107`](../config/runtime.exs#L107) |
...
| Variable | Default | Purpose | Where it's read |
|----------|---------|---------|-----------------|
| `OTEL_EXPORTER_OTLP_ENDPOINT` | unset | OTLP collector endpoint — anything that speaks OTLP: a local `otel-collector`, Grafana Alloy, Jaeger, or a vendor's ingest URL. Enables tracing when set. Port 4318 is the conventional HTTP port, 4317 gRPC; match it to the protocol below. | [`config/runtime.exs:117`](../config/runtime.exs#L117) |
| `OTEL_SERVICE_NAME` | `kiln_cms` | Service name attached to spans. Free-form — this is how the deployment is labelled in the tracing UI. | [`config/runtime.exs:123`](../config/runtime.exs#L123) |
| `OTEL_EXPORTER_OTLP_PROTOCOL` | `http_protobuf` | OTLP protocol: `http_protobuf`, `http_json` or `grpc`. | [`config/runtime.exs:127`](../config/runtime.exs#L127) |
...
| `KILN_FEDERATION_ENABLED` | `false` | Set to `true`/`1`/`yes`/`on` to let sites on this deployment be followed from Mastodon and other fediverse servers (#491). This is the **deployment** half of a two-part gate: with it unset, every federation route `404`s no matter what a site admin enabled. Turning it on means the server signs and POSTs to hosts chosen by strangers who followed a site, indefinitely and unattended — if your egress policy forbids that, leave it alone, since no tenant admin can override it. A site still opts in individually (`mix kiln.federation enable`). Parsed by the shared [on/off rules](#onoff-variables). Ignored under `MIX_ENV=test`. See [federation.md](federation.md). | [`config/runtime.exs:373`](../config/runtime.exs#L373) |
| `KILN_EXPERIMENTS_ENABLED` | `false` | Set to `true`/`1`/`yes`/`on` to serve A/B content experiments (#499). OFF by default, and the deployment gets a say because a page under a running experiment is served `private, no-store` — with the usual `public, max-age=60` a CDN would cache one arm and hand it to every visitor, which is a 100/0 split reported as 50/50. No visitor is tracked unless you also set `KILN_EXPERIMENTS_STICKY` below: on-site assignment is stateless and headless callers supply their own `?variant_key=`. Parsed by the shared [on/off rules](#onoff-variables). Ignored under `MIX_ENV=test`. See [content-experiments-plan.md](content-experiments-plan.md). | [`config/runtime.exs:383`](../config/runtime.exs#L383) |
| `KILN_EXPERIMENTS_STICKY` | `false` | Set to `true`/`1`/`yes`/`on` to keep a visitor in the same A/B arm across page loads (#984). **This is the one switch that puts a cookie on visitors**, so it is deliberately separate from `KILN_EXPERIMENTS_ENABLED` rather than implied by it. What is stored is a *bucket* — one integer in `0..99`, shared by a crowd, joined to nothing server-side — plus, for a `content_view` goal, a second cookie `_kiln_ab_x` naming the arm the visitor was shown (up to 4, each cleared the moment it converts) — the weaker of the two claims, since a combination of arms starts to narrow a visitor down. Both are `__Host-`-prefixed wherever your cookies are `Secure`, and are only minted on a page actually under experiment. Whether your regime wants consent for it is your call; see [data-flows.md](data-flows.md#sticky-assignment-cookie-984). Parsed by the shared [on/off rules](#onoff-variables). | [`config/runtime.exs:393`](../config/runtime.exs#L393) |
| `KILN_EXPERIMENTS_STICKY_DAYS` | `30` | Lifetime of the sticky cookies above, in days. Long enough to outlive the experiment a visitor is in, short enough not to be a standing marker — a year is the reflex default and would be one. A non-positive or unparseable value warns and keeps the default. | [`config/runtime.exs:399`](../config/runtime.exs#L399) |
| `KILN_PROVENANCE_SIGNER` | `:site_name` | Human-readable signer identity recorded in every manifest a consumer verifies — part of what the deployment publicly asserts about its content (#644). Unset falls back to the site name (which a released image can already set), so it is lower stakes than the three above; set it only to override the claim's signer without a rebuild. A blank value counts as unset. Ignored under `MIX_ENV=test`. | [`config/runtime.exs:1186`](../config/runtime.exs#L1186) |
| `KILN_PROVENANCE_ORIGIN` | `:public_base_url` | Origin URL recorded in the manifest claim (#644). Unset falls back to the public base URL a released image already configures; set it only to override. A blank value counts as unset. Ignored under `MIX_ENV=test`. | [`config/runtime.exs:1192`](../config/runtime.exs#L1192) |
| `KILN_PROVENANCE_AI_DISCLOSURE` | `human` | Default AI-generation disclosure embedded when a document declares none (an editor can override per-document via `custom_fields["ai_disclosure"]`). One of `human`/`ai_assisted`/`ai_generated` (case-insensitive). **An unrecognized value warns and keeps the default** rather than being coerced — it rides into a signed claim, so a typo must not silently rewrite what the deployment asserts (#644). A blank value counts as unset. Ignored under `MIX_ENV=test`. | [`config/runtime.exs:1205`](../config/runtime.exs#L1205) |
...
| Variable | Default | Purpose | Where it's read |
|----------|---------|---------|-----------------|
| `KILN_UPDATE_CHECK` | enabled | Set to an off-spelling for an instance that must make no outbound requests. | [`config/runtime.exs:499`](../config/runtime.exs#L499) |
| `KILN_UPDATE_REPO` | `The-Verscienta/kiln_cms` | The `owner/name` this build compares itself against. **Forks must set this.** Left at the default, a fork is told about upstream's releases — and a fork *ahead* of upstream compares as newer, so the page reports "Up to date" forever and the fork's own security releases never surface. A value that isn't `owner/name` is rejected, not ignored. | [`config/runtime.exs:529`](../config/runtime.exs#L529), [`Kiln.Updates`](../lib/kiln/updates.ex) |
| `KILN_UPDATE_RELEASES_URL` | derived from `KILN_UPDATE_REPO` | Full releases-API endpoint, for GitHub Enterprise or an internal mirror that can't reach `api.github.com`. Overrides the endpoint only — set `KILN_UPDATE_REPO` alongside it so the release link has a fallback. | [`config/runtime.exs:535`](../config/runtime.exs#L535), [`Kiln.Updates`](../lib/kiln/updates.ex) |
| `KILN_PIN_PATH` | unset | Path to this project's pinned Kiln checkout (`kiln/upstream`, `upstream`, …). Display only: the update page prefixes its `mix kiln.update` command with a matching `cd`. Unset by default because the pin's path is a downstream choice — see [`projects/README.md`](../projects/README.md). | [`config/runtime.exs:512`](../config/runtime.exs#L512), [`Kiln.Updates`](../lib/kiln/updates.ex) |
...
| Variable | Default | Purpose | Where it's read |
|----------|---------|---------|-----------------|
| `ADMIN_EMAIL` | `admin@kiln.test` | Email of the admin user created by `mix run priv/repo/seeds.exs`. | [`priv/repo/seeds.exs:11`](../priv/repo/seeds.exs#L11) |
| `ADMIN_PASSWORD` | `kilnadmin123` | That user's password. **The default is a published credential** — override it for anything reachable from a network. | [`priv/repo/seeds.exs:11`](../priv/repo/seeds.exs#L11) |
| `EDITOR_EMAIL` | `editor@kiln.test` | Email of the seeded editor user. | [`priv/repo/seeds.exs:11`](../priv/repo/seeds.exs#L11) |
| `EDITOR_PASSWORD` | `kilneditor123` | That user's password; same caveat as `ADMIN_PASSWORD`. | [`priv/repo/seeds.exs:11`](../priv/repo/seeds.exs#L11) |
