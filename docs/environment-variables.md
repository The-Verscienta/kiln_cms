...
| `TENANT_BASE_HOST` | `PHX_HOST` | The apex multi-tenant subdomains are carved from (epic #336): a request to `<org-slug>.<TENANT_BASE_HOST>` resolves to that organization. Set it explicitly only when tenant subdomains live under a different apex than the canonical URL host. | [`config/runtime.exs:475`](../config/runtime.exs#L475) |
| `TENANT_STRICT_HOST` | `false` | When on, a request whose `Host` matches no organization gets a **404** instead of being served the default org (#563). Off keeps the single-host behaviour: a bare hostname, an IP literal, `localhost` or an attacker-supplied `Host` all resolve to the default organization. **Turn it on for any deployment serving more than one organization** — leaving it off there means an unrecognized `Host` is served one tenant's content, branding and analytics under another's name. Both health probes (`/up`, `/ready`) stay exempt so a load balancer or readiness gate checking by pod IP does not pull the instance out of rotation. The boundary is the endpoint plug, which runs *after* `Plug.Static` — so `/assets/*` and `/uploads/*` are still served to any host. See [`KilnCMSWeb.Tenant`](../lib/kiln_cms_web/tenant.ex). | [`config/runtime.exs:146`](../config/runtime.exs#L146) |
| `TENANT_STRICT_HOST_EXEMPT_PATHS` | `/up,/ready` | Comma-separated **exact** request paths that skip the `TENANT_STRICT_HOST` check. Replaces the default rather than extending it, so keep `/up,/ready` unless you mean to drop them. Only needed for something addressed by an internal name rather than a site hostname. | [`config/runtime.exs:155`](../config/runtime.exs#L155) |
