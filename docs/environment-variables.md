...
| `TOKEN_SIGNING_SECRET` | Signs authentication tokens (AshAuthentication). Raises if missing. | [`config/runtime.exs:886`](../config/runtime.exs#L886) |
...
| Variable | Default | Purpose | Where it's read |
|----------|---------|---------|-----------------|
| `SITE_NAME` | `KilnCMS` | Instance name in the admin chrome, page titles and outbound email. Also the default provenance `signer` identity when `KilnCMS.Provenance`'s `:signer` is unset. | [`config/runtime.exs:841`](../config/runtime.exs#L841) |
| `BRAND_LOGO_URL` | unset | Logo shown in the admin chrome and on branded error pages. If the host differs from the site's origin it must also be in `CSP_IMG_SRC`, or the browser blocks the image. | [`config/runtime.exs:848`](../config/runtime.exs#L848) |
| `BRAND_FAVICON_URL` | unset | Favicon URL. Same `CSP_IMG_SRC` caveat as the logo. | [`config/runtime.exs:855`](../config/runtime.exs#L855) |
...
| Variable | Default | Purpose | Where it's read |
|----------|---------|---------|-----------------|
| `S3_BUCKET` | unset | Enables the S3 adapter. Leave unset to use local storage. | [`config/runtime.exs:980`](../config/runtime.exs#L980) |
| `S3_PUBLIC_BASE_URL` | — | Public base URL objects are served from — the CDN hostname, including the bucket path if the provider's URLs carry one. **Required when `S3_BUCKET` is set** (raises otherwise). | [`config/runtime.exs:987`](../config/runtime.exs#L987) |
| `AWS_ACCESS_KEY_ID` | — | S3 access key. **Required when `S3_BUCKET` is set** (`fetch_env!`). | [`config/runtime.exs:1013`](../config/runtime.exs#L1013) |
| `AWS_SECRET_ACCESS_KEY` | — | S3 secret key. **Required when `S3_BUCKET` is set** (`fetch_env!`). | [`config/runtime.exs:1014`](../config/runtime.exs#L1014) |
| `AWS_REGION` | `us-east-1` | Region. Use `auto` for Cloudflare R2; a real region for B2/Wasabi/AWS. | [`config/runtime.exs:1016`](../config/runtime.exs#L1016) |
| `S3_ACL` | unset | Per-object canned ACL (e.g. `public_read`). Only needed if the bucket isn't public at the bucket level. | [`config/runtime.exs:994`](../config/runtime.exs#L994) |
| `S3_PRIVATE_BUCKET` | unset | A separate bucket for gated documents (#481) — this app's own AWS credentials read it directly, so it needs no public-read config, CDN, or public-base-URL equivalent. Without it, gating a document is refused rather than silently falling back to the public bucket. | [`config/runtime.exs:1005`](../config/runtime.exs#L1005) |
| `S3_ENDPOINT_HOST` | unset | Custom endpoint host for non-AWS stores (R2/B2/Wasabi/MinIO). Leave unset for AWS S3. | [`config/runtime.exs:1020`](../config/runtime.exs#L1020) |
| `S3_ENDPOINT_SCHEME` | `https://` | Scheme for the custom endpoint. | [`config/runtime.exs:1022`](../config/runtime.exs#L1022) |
| `S3_ENDPOINT_PORT` | `443` | Port for the custom endpoint. | [`config/runtime.exs:1024`](../config/runtime.exs#L1024) |
...
| Variable | Default | Purpose | Where it's read |
|----------|---------|---------|-----------------|
| `BACKUP_ENABLED` | `true` | Set to an off-spelling to disable the **in-app** backup path — the console explains why and the button is disabled. Cron backups on the host are unaffected, which is the point: turning off the button is not turning off backups. Parsed by the shared [on/off rules](#onoff-variables). | [`config/runtime.exs:939`](../config/runtime.exs#L939) |
| `BACKUP_DIR` | `/var/backups/kiln` | Where backups land, for both paths. | [`config/runtime.exs:940`](../config/runtime.exs#L940) |
| `BACKUP_KEEP_DAYS` | `14` | Local retention in days, enforced by both paths. A non-positive or unparseable value keeps the default and warns — read literally, `0` would delete the backup it had just taken. | [`config/runtime.exs:941`](../config/runtime.exs#L941) |
| `BACKUP_STALE_AFTER_HOURS` | `36` | How old the newest backup may be before the console warns and the overview shows a red strip. Deliberately longer than a daily cadence: a warning that fires because a nightly job ran at 03:20 instead of 03:17 is one an admin learns to ignore. | [`config/runtime.exs:942`](../config/runtime.exs#L942) |
| `MEDIA_DIR` | unset | Uploads root to archive — **Local storage adapter only**. Leave unset on S3/R2, where the bucket is backed up provider-side: tarring a directory that doesn't hold the media produces an archive that looks like a media backup and restores nothing. | [`config/runtime.exs:968`](../config/runtime.exs#L968) |
...
| Variable | Default | Purpose | Where it's read |
|----------|---------|---------|-----------------|
| `MAIL_MODE` | unset | `smtp` = relay through an SMTP server; `direct` = deliver straight to each recipient domain's MX hosts (built-in MTA, no relay). Anything else raises at boot. | [`config/runtime.exs:1201`](../config/runtime.exs#L1201) |
| `MAIL_FROM_EMAIL` | unset | From address for all outbound mail. **Required when `MAIL_MODE=direct`** (raises otherwise) — its domain is the sending/DKIM domain. | [`config/runtime.exs:1241`](../config/runtime.exs#L1241) |
| `MAIL_FROM_NAME` | `KilnCMS` | Display name for the From address. | [`config/runtime.exs:1273`](../config/runtime.exs#L1273) |
| `SMTP_HOST` | unset | Relay host. **Required when `MAIL_MODE=smtp`**; setting it without `MAIL_MODE` also selects smtp mode. | [`config/runtime.exs:1202`](../config/runtime.exs#L1202) |
| `SMTP_PORT` | `587` | Relay port. 587 (STARTTLS) is the right default, and 25 belongs to `MAIL_MODE=direct`. **465 will not work**: that port expects implicit TLS from the first byte, and the adapter is configured for STARTTLS only (`tls:`, never gen_smtp's `ssl:`) with no environment variable to change it — a relay that offers both ports should be pointed at 587. | [`config/runtime.exs:1233`](../config/runtime.exs#L1233) |
| `SMTP_USERNAME` | unset | Relay username (`auth: :always`). **From the relay provider's dashboard** — providers name the pair differently: Postmark issues one Server API Token used as *both* username and password; SES issues dedicated SMTP credentials, which are **not** your AWS access keys; Gmail requires an App Password rather than the account password. | [`config/runtime.exs:1234`](../config/runtime.exs#L1234) |
| `SMTP_PASSWORD` | unset | Relay password; see `SMTP_USERNAME` for where it comes from. | [`config/runtime.exs:1235`](../config/runtime.exs#L1235) |
| `SMTP_TLS` | `true` | STARTTLS to the relay. Set to an off-spelling only for a local dev/test relay. | [`config/runtime.exs:1219`](../config/runtime.exs#L1219) |
| `SMTP_TLS_VERIFY` | `true` | Verify the relay's certificate against [CAStore](https://hex.pm/packages/castore)'s bundle, with SNI. Set to an off-spelling for a relay with a self-signed or mismatched certificate: the connection stays encrypted but the peer is not verified (`verify_none`). | [`config/runtime.exs:1219`](../config/runtime.exs#L1219) |
| `MAIL_HELO_HOST` | `PHX_HOST` | Direct mode only: HELO/EHLO hostname. Deliverability requires the sending IP's PTR record to resolve to this name. | [`config/runtime.exs:1249`](../config/runtime.exs#L1249) |
...
| Variable | Default | Purpose | Where it's read |
|----------|---------|---------|-----------------|
| `MEILI_URL` | unset | Meilisearch server URL. Enables the backend when set. | [`config/runtime.exs:1033`](../config/runtime.exs#L1033) |
| `MEILI_MASTER_KEY` | unset | Meilisearch API master key. | [`config/runtime.exs:1037`](../config/runtime.exs#L1037), [`lib/kiln_cms/search/meilisearch.ex:14`](../lib/kiln_cms/search/meilisearch.ex#L14) |
| `MEILI_INDEX` | `kiln_content` | Index name. | [`config/runtime.exs:1038`](../config/runtime.exs#L1038) |
...
| Variable | Default | Purpose | Where it's read |
|----------|---------|---------|-----------------|
| `SEO_MODEL` | unset | `req_llm` model spec, e.g. `ollama:llama3.1` or `anthropic:claude-sonnet-5`. Enables drafting when set. | [`config/runtime.exs:1053`](../config/runtime.exs#L1053) |
| `SEO_GENERATOR` | `KilnCMS.Seo.Generator.ReqLLM` | Override the adapter module with your own `KilnCMS.Seo.Generator`. | [`config/runtime.exs:1055`](../config/runtime.exs#L1055) |
...
| Variable | Default | Purpose | Where it's read |
|----------|---------|---------|-----------------|
| `ASSIST_MODEL` | unset | `req_llm` model spec, e.g. `ollama:llama3.1`. Enables block assist when set. | [`config/runtime.exs:1076`](../config/runtime.exs#L1076) |
| `ASSIST_GENERATOR` | `KilnCMS.Assist.Generator.ReqLLM` | Override the adapter module with your own `KilnCMS.Assist.Generator`. | [`config/runtime.exs:1078`](../config/runtime.exs#L1078) |
...
| Variable | Default | Purpose | Where it's read |
|----------|---------|---------|-----------------|
| `ASK_MODEL` | unset | `req_llm` model spec, e.g. `ollama:llama3.1`. Enables generated answers when set. | [`config/runtime.exs:1104`](../config/runtime.exs#L1104) |
| `ASK_GENERATOR` | `KilnCMS.Ask.Generator.ReqLLM` | Override the adapter module with your own `KilnCMS.Ask.Generator`. | [`config/runtime.exs:1106`](../config/runtime.exs#L1106) |
...
| Variable | Default | Purpose | Where it's read |
|----------|---------|---------|-----------------|
| `OEMBED_ENABLED` | `false` | Resolve oEmbed metadata so embeds render as cards. | [`config/runtime.exs:1133`](../config/runtime.exs#L1133) |
| `OEMBED_PROVIDERS` | unset (all) | Comma-separated provider names to **narrow** the built-in list. Cannot add one — a new provider is a host this server dials, so it is a code change. | [`config/runtime.exs:1135`](../config/runtime.exs#L1135) |
...
| `KILN_PROVENANCE_SIGNER` | `:site_name` | Human-readable signer identity recorded in every manifest a consumer verifies — part of what the deployment publicly asserts about its content (#644). Unset falls back to the site name (which a released image can already set), so it is lower stakes than the three above; set it only to override the claim's signer without a rebuild. A blank value counts as unset. Ignored under `MIX_ENV=test`. | [`config/runtime.exs:1295`](../config/runtime.exs#L1295) |
| `KILN_PROVENANCE_ORIGIN` | `:public_base_url` | Origin URL recorded in the manifest claim (#644). Unset falls back to the public base URL a released image already configures; set it only to override. A blank value counts as unset. Ignored under `MIX_ENV=test`. | [`config/runtime.exs:1301`](../config/runtime.exs#L1301) |
| `KILN_PROVENANCE_AI_DISCLOSURE` | `human` | Default AI-generation disclosure embedded when a document declares none (an editor can override per-document via `custom_fields["ai_disclosure"]`). One of `human`/`ai_assisted`/`ai_generated` (case-insensitive). **An unrecognized value warns and keeps the default** rather than being coerced — it rides into a signed claim, so a typo must not silently rewrite what the deployment asserts (#644). A blank value counts as unset. Ignored under `MIX_ENV=test`. | [`config/runtime.exs:1322`](../config/runtime.exs#L1322) |
