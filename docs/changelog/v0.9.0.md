...
The long-form entries behind the 0.9.0 section of
...
<a id="semantic-search-now-needs-an-image-built-with-kilnml1-upgrading"></a>

**Semantic search now needs an image built with `KILN_ML=1`.** The ML stack
behind it (Bumblebee, Nx, EXLA) is no longer compiled in by default (#1474), and
that includes the published `ghcr.io/the-verscienta/kiln_cms` image, which is
built without it. If your deployment has semantic search on (`semantic: true`):

1. Build your own image with `docker build --build-arg KILN_ML=1 …`, or for a
   source deployment set `KILN_ML=1` for every `mix` command
   (`KILN_ML=1 mix deps.get && KILN_ML=1 mix compile`, and the release build).
2. Deploy that image rather than the published `0.9.0` / `latest` tags.

Skip this and nothing crashes: search falls back to its keyword legs, and the
boot log warns once that the build cannot serve semantic search. Nothing in the
database or configuration changes. A deployment with semantic search off (the
...
<a id="a-temporary-role-is-applied-where-a-tier-is-decided-role-on-a-read-is-always"></a>

- **A temporary role is applied where a tier is decided; `role` on a read is always the standing tier.**
  ([#1462](https://github.com/The-Verscienta/kiln_cms/issues/1462) · [Account administration](../account-administration.md))

<a id="account-removal-no-longer-scans-every-admin-defined-entry-per-type"></a>
...
<a id="task-emails-for-content-of-a-deleted-content-type-are-sent-instead-of-crashing"></a>
...
<a id="a-system-actor-publish-or-membership-transition-no-longer-crashes-on-the-actors"></a>
