...
(rich text via TipTap with a floating toolbar). Edits push back over the socket
and write through the same Ash `:update` / `:autosave` actions the block editor
uses, so policies (#332) and paper-trail versioning are native. Reachable via
"Edit on page" from the structured editor. Structural add/reorder/delete stays in
the block editor / page-building (#335) and is out of scope here.
