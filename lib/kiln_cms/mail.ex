    ...
    case Mailer.deliver(email, config) do
      ...
      {:error, reason} ->
        ...
        case failure_class(reason) do
          :recipient -> cancel_permanent(email, safe_reason, true)
          :message -> cancel_permanent(email, safe_reason, false)
          :relay -> retry_relay_refused(email, safe_reason)
          :transient -> retry_transient(email, reason, safe_reason)
        end
    end
  end

  # A hard 5xx: log + emit a bounce event + (if it names the recipient)
  # suppress the address, then cancel.
  defp cancel_permanent(email, safe_reason, suppress?) do
    ...
    # Remember the dead address so future sends skip it (enqueue!) — only on a
    # reject that says the address is dead. The list is instance-wide and
    # outlives this message, so a guess here silently stops a person's password
    # resets until an admin notices.
    if suppress?, do: suppress_recipients(email, safe_reason)
