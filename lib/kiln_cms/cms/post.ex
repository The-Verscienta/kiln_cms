defmodule KilnCMS.CMS.Post do
  ...
  actions do
    ...
    # Full-text search over the denormalized `search_text` (title + excerpt +
    # SEO + block text). Goes through the read policy, so anonymous callers
    ...
    # Public delivery: fetch a single published post by slug. The `state ==
    ...
    # Public delivery: published posts, newest first (the blog index).
    read :published do
  ...
  attributes do
    ...
    # Denormalized plain-text (title + excerpt + SEO + block text) maintained by
  ...
  relationships do
    # The user who authored this post. Nullable so existing/system content
