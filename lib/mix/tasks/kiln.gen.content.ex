    ...
    * `--from <name>` - **promote an admin-defined dynamic type** (decision
    ...
    def interfaces(type, plural, opts) do
      base = [
        ...
        {:"save_#{type}_working_copy",
         "define :save_#{type}_working_copy, action: :save_working_copy"},
        {:"publish_#{type}_changes", "define :publish_#{type}_changes, action: :publish_changes"},
        {:"discard_#{type}_changes", "define :discard_#{type}_changes, action: :discard_changes"},
