defmodule Mix.Tasks.Kiln.Search.MeasureFloor do
  ...
  fusion — a record any other leg (keyword, title, fuzzy) also returns is never floored —
  ...
  defp advice(expected, junk) do
    ...
    if furthest.distance < nearest.distance do
      ...
      "Suggested semantic_max_distance: #{fmt(midpoint)}\n" <>
        ...
        "  A hit any other leg (keyword, title, fuzzy) also returns is never floored by hybrid\n" <>
        "  search, so if the gap is thin, favour the junk edge.\n"
    else
