...
- The checkout's path must not contain a space. `picosat_elixir` (Ash's policy SAT solver) is the one dep that cannot compile there: its Makefile uses absolute paths as make targets, and make splits them at the space. The error blames a missing gcc/make, which is wrong. `mix setup` refuses such a path up front. The fix is [bitwalker/picosat_elixir#14](https://github.com/bitwalker/picosat_elixir/pull/14), not yet released. Every other native dep (bcrypt_elixir, vix/libvips, y_ex) builds under a spaced path.
- Keep the `igniter` dep: `mix kiln.gen.content` and `mix kiln.gen.plugin` are Igniter tasks, and their tests use `Igniter.Test`. Removing it does not crash the compiler.
- **The ML stack is opt-in (`KILN_ML=1`).** Bumblebee/Nx/EXLA are left out of the dependency tree unless `KILN_ML` is on — `deps/exla` is 666 MB of a 773 MB tree (102 MB without it), for a semantic search that ships disabled. A default build is therefore a *lean* build: `KilnCMS.Search.ML.available?/0` is false, the Bumblebee adapters return `{:error, %KilnCMS.Search.ML.NotCompiledError{}}`, and search falls back to its keyword legs. Touching anything on the semantic path (`lib/kiln_cms/search/serving.ex`, `reranker_serving.ex`, either `.../bumblebee.ex` adapter, `KilnCMS.Application`'s serving children) means editing **two compiled shapes** — branch in the module body, never at runtime; `available?/0` is a compile-time constant and Elixir's type checker rejects a runtime `if` on it. To work on that path for real: `KILN_ML=1 mix deps.get && KILN_ML=1 mix compile`, and keep the variable exported.
