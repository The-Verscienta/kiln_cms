defmodule KilnCMS.Repo.Migrations.BackfillDefinitionsOrg do
  @moduledoc """
  Data migration for the multi-tenancy rollout (epic #336), PR 4b — **step 2 of
  3** for the dynamic type-registry tables.

  Runs between the two schema migrations that add `org_id` to
  `type_definitions`/`field_definitions`:
    1. `add_definitions_nullable_org_id` — adds the **nullable** `org_id`.
    ...
    3. `require_definitions_org_id_not_null` — flips `org_id` to `NOT NULL`.
  ...
  @scoped_tables ~w(type_definitions field_definitions)
